...
namespace API.Controllers
{
    ...
    public class ca_fieldsController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_ca_fields(doc_ca_fields ca_fields)
        {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_ca_fields(doc_ca_fields ca_fields)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "ca_fields/Update_ca_places", ip, tid, "Lỗi khi cập nhật ca_fields", 0, "ca_fields");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_ca_fields([FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "ca_fields/Delete_ca_places", ip, tid, "Lỗi khi xoá ca_fields", 0, "ca_fields");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusFields(Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var das = db.doc_ca_fields.Where(a => (a.field_id == trangthai.IntID)).FirstOrDefault<doc_ca_fields>();
                        if (das != null)
                        {
