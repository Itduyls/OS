...
namespace API.Controllers
{
    ...
    public class ca_email_groupsController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_email_groups(doc_ca_email_groups email_Groups)
        {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_email_groups(doc_ca_email_groups email_Groups)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "email_Groups/Update_ca_places", ip, tid, "Lỗi khi cập nhật Nhóm email", 0, "email_Groups");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_email_groups([FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "email_Groups/Delete_ca_places", ip, tid, "Lỗi khi xoá nhóm email", 0, "email_Groups");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusGroup_Email(Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
