...
namespace API.Controllers
{
    ...
    public class ca_dispatch_booksController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_dispatch_book(doc_ca_dispatch_books dispatch_Books)
        {
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_dispatch_books(doc_ca_dispatch_books dispatch_Books)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            {
                string contents = helper.getCatchError(e, null);
                helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = fdlang, contents }), domainurl + "dispatch_Books/Update_ca_places", ip, tid, "Lỗi khi cập nhật Công văn", 0, "dispatch_Books");
                if (!helper.debug)
                {
                    contents = "";
                }
        ...
        [HttpDelete]
        public async Task<HttpResponseMessage> Delete_dispatch_books([FromBody] List<int> id)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
            ...
            try
            {
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = id, contents }), domainurl + "dispatch_Books/Delete_ca_places", ip, tid, "Lỗi khi xoá công văn", 0, "dispatch_Books");
                    if (!helper.debug)
                    {
                        contents = "";
                    }
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_StatusDispatch_Books(Trangthai trangthai)
        {
            var identity = User.Identity as ClaimsIdentity;
            IEnumerable<Claim> claims = identity.Claims;
            try
