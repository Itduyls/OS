...
namespace API.Controllers
{
    ...
    public class positionsController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> Add_ca_positions(ca_positions ca_Positions)
        {
            ...
            try
            {
                ...
                {
                    ca_Positions.modified_by = name;
                    ca_Positions.modified_date = DateTime.Now;
                    ca_Positions.modified_ip = ip;
                    ca_Positions.modified_token_id = tid;
                    db.ca_positions.Add(ca_Positions);
                    ...
                    if (helper.wlog)
                    {
                        ...
                        log.log_title = "Thêm chức vụ " + ca_Positions.position_name;
        ...
        [HttpPut]
        public async Task<HttpResponseMessage> Update_ca_positions(ca_positions ca_Positions)
        {
            var identity = User.Identity as ClaimsIdentity;
            string fdlang = "";
            IEnumerable<Claim> claims = identity.Claims;
            ...
            try
            {
                using (DBEntities db = new DBEntities())
                {
                    ...
                    if (helper.wlog)
                    {
                        ...
                        cms_logs log = new cms_logs();
                        log.log_title = "Sửa chức vụ " + ca_Positions.position_name;
                        log.log_content = JsonConvert.SerializeObject(new { data = ca_Positions });
                        log.log_module = "Chức vụ";
                        log.id_key = ca_Positions.position_id.ToString();
        ...
        public async Task<HttpResponseMessage> Delete_ca_positions([FromBody] List<int> id)
        {
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var das = await db.ca_positions.Where(a => id.Contains(a.position_id)).ToListAsync();

                        if (das != null)
                        {
                            List<ca_positions> del = new List<ca_positions>();
                            foreach (var da in das)
                            {
                                ...
                                if (helper.wlog)
                                {
                                    ...
                                    cms_logs log = new cms_logs();
                                    log.log_title = "Xóa chức vụ" + da.position_name;
                            ...
                            db.ca_positions.RemoveRange(del);
                        }
                        await db.SaveChangesAsync();
        ...
        public async Task<HttpResponseMessage> Update_StatusPositions(Trangthai trangthai)
        {
            ...
            try
            {
                ...
                try
                {
                    using (DBEntities db = new DBEntities())
                    {
                        var das = db.ca_positions.Where(a => (a.position_id == trangthai.IntID)).FirstOrDefault<ca_positions>();
                        if (das != null)
                        {
                            ...
                            if (helper.wlog)
                            {
                                ...
                                cms_logs log = new cms_logs();
                                log.log_title = "Sửa trạng thái chức vụ" + das.position_name;
