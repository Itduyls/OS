...
namespace Controllers
{
    ...
    public class ProcController : ApiController
    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> AddUserLog(cms_logs log)
        {
            using (DBEntities db = new DBEntities())
            {
                ...
                catch (DbEntityValidationException e)
                {
                    ...
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = log, contents }), domainurl + "Proc/AddUserLog", ip, tid, "Lỗi khi Thêm Userlog", 0, "Proc");
                    if (!helper.debug)
                    {
        ...
        [HttpPost]
        public async Task<HttpResponseMessage> AddLog(sys_logs log)
        {
            using (DBEntities db = new DBEntities())
            {
                var identity = User.Identity as ClaimsIdentity;
        ...
        public async Task<HttpResponseMessage> CallProc([FromBody] sqlProc proc)
        {
            ...
            try
            {
                ...
                {
                    string contents = helper.getCatchError(e, null);
                    helper.saveLog(uid, name, JsonConvert.SerializeObject(new { data = proc, contents }), domainurl + "Proc/CallProc", ip, tid, "Lỗi khi gọi proc '" + proc + "'", 0, "Proc");
                    if (!helper.debug)
                    {
                        contents = helper.logCongtent;
                    }
