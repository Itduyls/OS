...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].issue_place_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
  ...
  axios({
    ...
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Cập nhật nơi ban hành thành công!");
        loadData(true);
        closeDialog();
      } else {
        swal.fire({
...
const onSort = (event) => {
  ...
  if (event.sortField != "issue_place_id") {
    options.value.sort +=
      ",issue_place_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Issue_Places", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  ...
  axios
    .put(baseURL + "/api/ca_issue_places/Update_StatusIssue_Place", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
...
<template>
  <div class="surface-100">
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-briefcase"></i> Danh sách nơi ban hành ({{
          ...
          <div class="col-12 p-0">
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber v-model="issuePlace.is_order" class="col-6 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch v-model="issuePlace.status" class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
