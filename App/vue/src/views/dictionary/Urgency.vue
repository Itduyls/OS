    ...
    $errors: [
      {
        ...
        $validator: "required",
        $message: "Tên độ khẩn không được để trống!",
      },
    ],
  },
};
...
const v$ = useVuelidate(rules, urgency);
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].urgency_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].urgency_id;
    options.value.IsNext = false;
  }
...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_urgency/${
        issaveField.value ? "Update_urgency" : "Add_urgency"
      }`,
    data: urgency.value,
    headers: {
    ...
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Cập nhật độ khẩn thành công!");
        loadData(true);
        closeDialog();
      } else {
        swal.fire({
...
const delField = (Field) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá độ khẩn này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/ca_urgency/Delete_urgency", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Field != null ? [Field.urgency_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá độ khẩn thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const onSort = (event) => {
  ...
  if (event.sortField != "urgency_id") {
    options.value.sort +=
      ",urgency_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Urgency", data, config)
    .then((response) => {
...
const onFilter = (event) => {
  ...
  for (const [key, value] of Object.entries(event.filters)) {
    if (key != "global") {
      let obj = {
        key: key != "urgency_name" ? "urgency_name" : key,
...
const onCheckBox = (value) => {
  let data = {
    IntID: value.urgency_id,
    TextID: value.urgency_id + "",
    IntTrangthai: 1,
...
<template>
  <div class="surface-100">
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-bolt"></i> Danh sách độ khẩn ({{
    ...
    <div class="d-lang-table mx-3">
      <DataTable
        ...
        :globalFilterFields="['urgency_name']"
      >
        <Column
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            ...
            <div class="col-12 flex p-0">
              ...
              <small
                v-if="
                  (v$.urgency_name.$invalid && submitted) ||
                  v$.urgency_name.$pending.$response
                "
                class="col-9 p-error p-0"
              >
                <span class="col-12 p-0">{{
                  v$.urgency_name.required.$message
                    .replace("Value", "Tên độ khẩn")
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber v-model="urgency.is_order" class="col-6 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch v-model="urgency.status" class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
