...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id =
      datalists.value[datalists.value.length - 1].signer_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
...
const onSort = (event) => {
  ...
  if (event.sortField != "signer_id") {
    options.value.sort += ",signer_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const onCheckBox = (value) => {
  ...
  axios
    .put(
      baseURL + "/api/ca_signers/Update_StatusSigner",
      data,
      config
    )
    .then((response) => {
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            <div class="col-12 p-0 flex my-2">
              ...
              <InputText v-model="signer.signer_name" spellcheck="false" class="col-10 p-0 m-0 ip36 px-2" />
            </div>
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber  v-model="signer.is_order" class="col-8 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
               
                <div class="pb-2 col-4 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch   v-model="signer.status"  class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
