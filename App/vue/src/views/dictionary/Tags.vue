...
const v$ = useVuelidate(rules, caTag);
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].tag_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].tag_id;
    options.value.IsNext = false;
  }
...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_tags/${
        issaveField.value ? "Update_tag" : "Add_tag"
      }`,
    data: caTag.value,
    headers: {
...
const editField = (dataPlace) => {
  submitted.value = false;
  caTag.value = dataPlace;
  headerDialog.value = "Sửa nhãn";
...
const delField = (Field) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá nhãn này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/ca_tags/Delete_tag", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Field != null ? [Field.tag_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
...
const exportData = (method) => {
  ...
  axios
    .post(
      ...
      {
        excelname: "DANH SÁCH NHÃN",
        proc: "ca_tag_listexport",
      },
      config
    )
    .then((response) => {
...
const onSort = (event) => {
  ...
  if (event.sortField != "tag_id") {
    options.value.sort +=
      ",tag_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Tag", data, config)
    .then((response) => {
...
const onCheckBox = (value) => {
  ...
  axios
    .put(baseURL + "/api/ca_tags/Update_Tag_Status", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
...
<template>
  <div class="surface-100">
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-send"></i> Danh sách nhãn ({{
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            <div class="col-12 p-0 flex my-2">
             ...
             Tên nhãn <span class="redsao">(*)</span>
              </div>
              <InputText
                v-model="caTag.tag_name"
                spellcheck="false"
            ...
            <div class="col-12 flex p-0">
              ...
              <small
                v-if="
                  (v$.tag_name.$invalid && submitted) ||
                  v$.tag_name.$pending.$response
                "
                class="col-9 p-error p-0"
              >
                <span class="col-12 p-0">{{
                  v$.tag_name.required.$message
                    .replace("Value", "Tên nhãn")
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber v-model="caTag.is_order" class="col-6 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch v-model="caTag.status" class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
