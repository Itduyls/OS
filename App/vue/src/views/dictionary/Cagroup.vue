          ...
          par: [
            ...
            { par: "status", va: options.value.Status },
             { par: "year", va: options.value.Year },
              { par: "code", va: options.value.Code },
          ],
        },
        config
      )
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].doc_group_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].doc_group_id;
    options.value.IsNext = false;
  }
...
const saveGroup = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_groups/${
        issaveGroup.value ? "Update_groups" : "Add_groups"
      }`,
    data: group.value,
    headers: {
    ...
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Cập nhật nhóm văn bản thành công!");
        loadData(true);
        closeDialog();
      } else {
        swal.fire({
...
const onSort = (event) => {
  ...
  if (event.sortField != "doc_group_id") {
    options.value.sort +=
      ",doc_group_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Groups", data, config)
    .then((response) => {
...
//Xóa nhiều
const deleteList = () => {
  let listId = new Array(selectedGroups.length);
  swal
    .fire({
      title: "Thông báo",
...
<template>
  <div class="surface-100">
          ...
          >
            <template #item="{ item }">
              <div class="grid formgrid m-2">
                <div class="group col-12 md:col-12">
                  ...
                  <span v-else>
                    ...
                    <Toolbar class="border-none surface-0 outline-none pb-0">
                      ...
                      <template #end>
                        <Button @click="filterGroups" label="Lọc"></Button>
                      </template>
                    </Toolbar>
                  </span>
                </div>
    ...
    <div class="d-lang-table mx-3">
      <DataTable
        ...
        :globalFilterFields="['doc_group_name']"
      >
        <Column
        ...
        >
          <template #body="Field">
            <Button
              @click="editGroup(Field.data)"
              class="p-button-rounded p-button-secondary p-button-outlined mx-1"
              type="button"
              icon="pi pi-pencil"
            ></Button>
            <Button
              @click="delGroup(Field.data, true)"
              class="p-button-rounded p-button-secondary p-button-outlined mx-1"
              type="button"
              icon="pi pi-trash"
            ></Button>
  ...
  >
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            <div class="col-12 p-0 flex my-2">
              ...
              <InputText v-model="group.doc_group_name" spellcheck="false" class="col-10 p-0 m-0 ip36 px-2" />
            </div>
            <div class="col-12 flex p-0">
             ...
             <div class="col-12 p-0 flex my-2">
              <div class="col-2 text-left p-0 pb-2 line-height-4">
                Mã nhóm <span class="redsao">(*)</span>
              </div>
              <InputText v-model="group.doc_group_code" spellcheck="false" class="col-10 p-0 m-0 ip36 px-2" />
            </div>
            <div class="col-12 flex p-0">
              <div class="col-2"></div>
               <small
            ...
            <div class="col-12 p-0 flex">
              <div class="col-6 p-0 my-3 flex">
                ...
                <InputNumber :useGrouping="false"  v-model="group.year"  spellcheck="false" class="col-8 p-0 ip36" />
              </div>
              <div class="col-6 p-0 my-3 flex">
                <label class="line-height-4 col-4">Số hiện tại</label>
                <InputNumber :useGrouping="false"  v-model="group.current_num"  spellcheck="false" class="col-8 p-0 ip36" />
              </div>
            </div>
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                <div class="pb-2 col-4 p-0 line-height-4">STT</div>
                <InputNumber v-model="group.is_order" class="col-8 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
               
                <div class="pb-2 col-4 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch   v-model="group.status"  class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
    ...
    <template #footer>
      ...
      <Button label="Lưu" icon="pi pi-check" @click="saveGroup(!v$.$invalid)" />
    </template>
  </Dialog>
</template>
