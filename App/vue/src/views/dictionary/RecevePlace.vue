...
const rules = {
  receive_place_name: {
    ...
    $errors: [
      {
        $property: "receive_place_name",
        $validator: "required",
        $message: "Tên nơi nhận không được để trống!",
      },
    ],
  },
};
const receive_places = ref({
  receive_place_name: "",
  status: true,
  is_order: 1,
});
...
const v$ = useVuelidate(rules, receive_places);
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].receive_place_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].receive_place_id;
    options.value.IsNext = false;
  }
...
const openBasic = (str) => {
  ...
  receive_places.value = {
    receive_place_name: "",
    is_order: sttField.value,
    status: true,
  };
  if (store.state.user.IsAdmin) {
    receive_places.value.organization_id = 0;
  }
...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_receive_places/${
        issaveField.value ? "Update_receive_places" : "Add_receive_place"
      }`,
    data: receive_places.value,
    headers: {
    ...
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Cập nhật nơi nhận thành công!");
        loadData(true);
        closeDialog();
      } else {
        swal.fire({
...
const editField = (dataPlace) => {
  submitted.value = false;
  receive_places.value = dataPlace;
  headerDialog.value = "Sửa nơi nhận";
...
const delField = (Field) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá nơi nhận này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/ca_receive_places/Delete_receive_places", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Field != null ? [Field.receive_place_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá nơi nhận thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const onSort = (event) => {
  ...
  if (event.sortField != "receive_place_id") {
    options.value.sort +=
      ",receive_place_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Receive", data, config)
    .then((response) => {
...
const onFilter = (event) => {
  ...
  for (const [key, value] of Object.entries(event.filters)) {
    if (key != "global") {
      let obj = {
        key: key != "receive_place_name" ? "receive_place_name" : key,
...
const onCheckBox = (value) => {
  let data = {
    IntID: value.receive_place_id,
    TextID: value.receive_place_id + "",
    IntTrangthai: 1,
    BitTrangthai: value.status,
  };
  axios
    .put(baseURL + "/api/ca_receive_places/Update_StatusReceive_Places", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        selectedFields.value.forEach((item) => {
          listId.push(item.receive_place_id);
        });
        axios
          .delete(baseURL + "/api/ca_receive_places/Delete_receive_places", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
...
<template>
  <div class="surface-100">
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-envelope"></i> Danh sách nơi nhận ({{
    ...
    <div class="d-lang-table mx-3">
      <DataTable
        ...
        :globalFilterFields="['receive_place_name']"
      >
        <Column
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            <div class="col-12 p-0 flex my-2">
              ...
              Nơi nhận<span class="redsao">(*)</span>
              </div>
              <InputText
                v-model="receive_places.receive_place_name"
                spellcheck="false"
            ...
            <div class="col-12 flex p-0">
              ...
              <small
                v-if="
                  (v$.receive_place_name.$invalid && submitted) ||
                  v$.receive_place_name.$pending.$response
                "
                class="col-9 p-error p-0"
              >
                <span class="col-12 p-0">{{
                  v$.receive_place_name.required.$message
                    .replace("Value", "Tên nơi nhận")
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber v-model="receive_places.is_order" class="col-6 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch v-model="receive_places.status" class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
