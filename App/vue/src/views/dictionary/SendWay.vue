...
const rules = {
  send_way_name: {
    ...
    $errors: [
      {
        $property: "send_way_name",
        $validator: "required",
        $message: "Tên hình thức gửi không được để trống!",
      },
    ],
  },
};
...
const v$ = useVuelidate(rules, send_ways);
...
const onPage = (event) => {
  if (event.page == 0) {
    ...
    options.value.id = datalists.value[datalists.value.length - 1].send_way_id;
    options.value.IsNext = true;
  } else if (event.page < options.value.PageNo) {
    //Trang trước
    options.value.id = datalists.value[0].send_way_id;
    options.value.IsNext = false;
  }
...
const openBasic = (str) => {
  ...
  if (store.state.user.IsAdmin) {
    send_ways.value.organization_id = 0;
  }
...
const saveField = (isFormValid) => {
  ...
  axios({
    ...
    url:
      baseURL +
      `/api/ca_send_ways/${
        issaveField.value ? "Update_send_ways" : "Add_send_way"
      }`,
    data: send_ways.value,
    headers: {
    ...
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
        toast.success("Cập nhật hình thức gửi thành công!");
        loadData(true);
        closeDialog();
      } else {
        swal.fire({
...
const editField = (dataPlace) => {
  submitted.value = false;
  send_ways.value = dataPlace;
  headerDialog.value = "Sửa hình thức gửi";
...
const delField = (Field) => {
  swal
    .fire({
      title: "Thông báo",
      text: "Bạn có muốn xoá hình thức gửi này không!",
      icon: "warning",
    ...
    .then((result) => {
      if (result.isConfirmed) {
        ...
        axios
          .delete(baseURL + "/api/ca_send_ways/Delete_send_ways", {
            headers: { Authorization: `Bearer ${store.getters.token}` },
            data: Field != null ? [Field.send_way_id] : 1,
          })
          .then((response) => {
            swal.close();
            if (response.data.err != "1") {
              swal.close();
              toast.success("Xoá hình thức gửi thành công!");
              loadData(true);
            } else {
              swal.fire({
                title: "Error!",
...
const exportData = (method) => {
  ...
  axios
    .post(
      ...
      {
        excelname: "DANH SÁCH HÌNH THỨC GỬI",
        proc: "ca_send_ways_listexport",
      },
      config
    )
    .then((response) => {
...
const onSort = (event) => {
  ...
  if (event.sortField != "send_way_id") {
    options.value.sort +=
      ",send_way_id " + (event.sortOrder == 1 ? " ASC" : " DESC");
  }
...
const loadDataSQL = () => {
  ...
  axios
    .post(baseURL + "/api/SQL/Filter_Send_ways", data, config)
    .then((response) => {
...
const onFilter = (event) => {
  ...
  for (const [key, value] of Object.entries(event.filters)) {
    if (key != "global") {
      let obj = {
        key: key != "send_way_name" ? "send_way_name" : key,
...
const onCheckBox = (value) => {
  let data = {
    IntID: value.send_way_id,
    TextID: value.send_way_id + "",
    IntTrangthai: 1,
    BitTrangthai: value.status,
  };
  axios
    .put(baseURL + "/api/ca_send_ways/Update_StatusSend_Way", data, config)
    .then((response) => {
      if (response.data.err != "1") {
        swal.close();
...
<template>
  <div class="surface-100">
    <div class="h-2rem p-3 pb-0 m-3 mb-0 surface-0">
      <h3 class="m-0">
        <i class="pi pi-send"></i> Danh sách hình thức gửi ({{
    ...
    <Toolbar class="outline-none mx-3 surface-0 border-none">
      ...
      <template #end>
        ...
        />
        <Button
          @click="openBasic('Thêm hình thức gửi')"
          label="Thêm mới"
          icon="pi pi-plus"
          class="mr-2"
        />
    ...
    <div class="d-lang-table mx-3">
      <DataTable
        ...
        :globalFilterFields="['send_way_name']"
      >
        <Column
        ...
        <div style="display: flex" class="field col-12 md:col-12">
          <div class="col-12 p-0">
            <div class="col-12 p-0 flex my-2">
              ...
              Hình thức gửi<span class="redsao">(*)</span>
              </div>
              <InputText
                v-model="send_ways.send_way_name"
                spellcheck="false"
            ...
            <div class="col-12 flex p-0">
              ...
              <small
                v-if="
                  (v$.send_way_name.$invalid && submitted) ||
                  v$.send_way_name.$pending.$response
                "
                class="col-9 p-error p-0"
              >
                <span class="col-12 p-0">{{
                  v$.send_way_name.required.$message
                    .replace("Value", "Tên hình thức gửi")
            ...
            <div class="col-12 p-0 my-3 flex">
              <div class="col-6 flex p-0">
                ...
                <InputNumber v-model="send_ways.is_order" class="col-6 p-0 ip36" />
              </div>
              <div class="col-6 flex p-0">
                <div class="pb-2 col-6 p-0 line-height-4 px-2">Trạng thái</div>
                <InputSwitch v-model="send_ways.status" class="col-6 p-0 ip36" />
              </div>
            </div>
          </div>
        </div>
